...
export default function Eventlist() {
  ...
  const token = '10ce9b985bdf252a91144e3b4a18c633cb293563';
    ...
    <div>
      ...
      {data.length > 0 ? (
        data.map((item, index) => (
            ...
            <div className='bg-white rounded-xl'>
            <Link to={`/${item.productId}`}><img src="https://media.istockphoto.com/id/501387734/fr/photo/danse-des-amis.jpg?s=612x612&w=0&k=20&c=uS-chxCnjK2DWqlUbT2qEp9yLzCp_3Q1PZq8XCbc99E=" alt="alt" className='w-screen rounded-t-xl'></img></Link>
              {/* <img src={item._links.self.href}></img> */}
              <p className='text-black text-2xl'>{item.eventName}</p>
              <p className='text-black'>{item.dateStart}</p>
            </div>
